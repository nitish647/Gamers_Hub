...
public class Connectivity {
    ...
    public static String getConnectionStatus(Context context) {
        NetworkInfo info = Connectivity.getNetworkInfo(context);
        if (info == null || !info.isConnected() || !isNetworkOnline1(context)) {
            return ConstantsHelper.ConnectionSignalStatus.NO_CONNECTIVITY+"";
        } else if (Connectivity.getInternetStatus(info.getType(), info.getSubtype(), context) == 3
                ) {
            return ConstantsHelper.ConnectionSignalStatus.GOOD_STRENGTH+"";
        } else if (Connectivity.getInternetStatus(info.getType(), info.getSubtype(), context) >= 2
                ) {
            return ConstantsHelper.ConnectionSignalStatus.FAIR_STRENGTH+"";
        }else {
            return ConstantsHelper.ConnectionSignalStatus.POOR_STRENGTH+"";
        }
    }
